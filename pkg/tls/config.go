...
func (c *ClientConfig) TLSConfig() (*tls.Config, error) {
	...
	if c.TLSCert != "" && c.TLSKey != "" {
		err := loadCertificate(tlsConfig, c.TLSCert, c.TLSKey, c.ServerName)
		if err != nil {
			return nil, err
		}
	}
...
func (c *ServerConfig) TLSConfig() (*tls.Config, error) {
	...
	if c.TLSCert != "" && c.TLSKey != "" {
		err := loadCertificate(tlsConfig, c.TLSCert, c.TLSKey, c.TLSAllowedDNSNames[0])
		if err != nil {
			return nil, err
		}
	}
...
func loadCertificate(config *tls.Config, certFile, keyFile, serverName string) error {
	...
	config.ServerName = serverName // SNI
	return nil
}
...
func (c *ServerConfig) verifyPeerCertificate(rawCerts [][]byte, _ [][]*x509.Certificate) error {
	...
	for _, name := range cert.DNSNames {
		if Contains(name, c.TLSAllowedDNSNames) {
			return nil
		}
	}
